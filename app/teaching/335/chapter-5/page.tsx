...
function storyBakery(n: string, t: number, r: string, P: number) {
  return `${n}: I'm fixing ovens in the old square and I'm asking to borrow ${money(P)} from you today to finish the hearth and tiles. I learned sweet rolls from my grandmother and I can scale dawn batches without burning a crumb. Market days bring lines that wrap the fountain; evenings we'll sell crusts to inns. If you hold the note for ${t} years, my tables price this near ${r} effective annually. Speak plainly about schedules and I'll speak plainly about cash. Bread cools fast; promises shouldn't—let's keep both warm.`;
}
function storyTea(n: string, t: number, r: string, P: number) {
  return `${n}: The river path needs a tea house and I’d like to borrow ${money(P)} from you for timber, braziers, and reed mats. Caravan tongues barter better after steam. My cousins will serve, I’ll count, and travelers will write about the quiet. If you hold for ${t} years the headline works to about ${r} a year. I keep a copper jar for repairs and a ledger for luck—one I trust more than the other. If you value punctual coins and clean books, pour with me.`;
}
function storyBoat(n: string, t: number, r: string, P: number) {
  return `${n}: I know the lake winds and I’m asking to borrow ${money(P)} from you to rig a broad‑keel boat for grain and stone. The west shore starves for ferries when storms sour roads. I price the voyage at ${r} annually over ${t} years if weather behaves and ropes don’t lie. I have crew who tie knots like poems and a habit of landing early. You lend steady; I’ll repay steady—plus a seat on the prow when the moon is a coin.`;
}
function storyVine(n: string, t: number, r: string, P: number) {
  return `${n}: Vines outside the wall swell with juice; I need to borrow ${money(P)} from you to buy barrels, presses, and a mule that doesn't gossip. Cellars want patience and roofs that don't drip. Over ${t} years the yield evens to ${r} give or take a late frost. My family bottles truthfully; labels brag, wine shouldn’t. If your appetite is for calm curves and honest sums, lend to me and I’ll repay on time.`;
}
function storySchool(n: string, t: number, r: string, P: number) {
  return `${n}: With ${money(P)} I can rent a sunny room, buy chalk and slates, and start a small after‑school class. I’m asking to borrow it from you and repay properly. Hold for ${t} years and reckon about ${r} effective; I post attendance like a merchant posts prices. If we count curiosity as collateral we’re already rich—but I still keep a lockbox and a ledger.`;
}
function storyWorkshop(n: string, t: number, r: string, P: number) {
  return `${n}: Wheels wobble, lamps fail, doors sulk; my workshop fixes the patience of the city. I want to borrow ${money(P)} from you for tools and stock, then keep the till honest. Over ${t} years a tidy operator makes about ${r} if nails stay cheap and apprentices stay curious. I’m partial to boring miracles: hinges that never squeak, balances that never lie, payments that arrive before you wonder.`;
}
function storyLump(n: string, t: number, _r: string, P: number, k: number) {
  const repay = P * k;
  return `${n}: I keep math simple. I’m borrowing ${money(P)} from you today so my plan can breathe. At the end of ${t} years I’ll repay ${money(repay)} in one clean payment—no fiddly coupons. Between, I keep my cash turning. We can write down the compounding if your heart insists, but my promise is simple: repay ${k.toFixed(2)}× the principal at maturity.`;
}
function storyMill(n: string, t: number, r: string, P: number) {
  return `${n}: There’s a mill by the falls that grinds hope into flour. Its stones are sharp but its beams sulk; I’m asking to borrow ${money(P)} from you to buy braces, belts, and paint the color of confidence. Give me ${t} years and I can work to about ${r} if harvests behave. The baker, the brewer, and I prefer reliable circles: millstones, coin cycles, seasons turning.`;
}
...
function makeLender(i: number): Lender {
  ...
  if (kind === "APR") {
    ...
    for (let tries = 0; tries < 8; tries++) {
      ...
      const earTrial = (1 + apr / m) ** m - 1;
      if (earTrial <= 0.15) { rate = apr; break; }
    }
    if (rate == null) rate = 0.12;
  } else if (kind === "EAR") {
    rate = rnd(0.03, 0.15, 4);
  } else {
